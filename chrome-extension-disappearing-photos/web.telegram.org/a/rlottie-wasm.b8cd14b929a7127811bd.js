...
ENVIRONMENT_IS_NODE ? (scriptDirectory = ENVIRONMENT_IS_WORKER ? require("path").dirname(scriptDirectory) + "/" : __dirname + "/", read_ = function(e, r) {
    return nodeFS || (nodeFS = require("fs")), nodePath || (nodePath = require("path")), e = nodePath.normalize(e), nodeFS.readFileSync(e, r ? null : "utf8")
...
function dynamicAlloc(e) {
    var r = HEAP32[DYNAMICTOP_PTR >> 2],
        t = r + e + 15 & -16;
    return HEAP32[DYNAMICTOP_PTR >> 2] = t, r
}
    ...
    switch (e) {
        ...
        default:
            ...
            if ("i" === e[0]) {
                ...
                return assert(r % 8 == 0, "getNativeTypeSize invalid bits " + r + ", type " + e), r / 8
            }
            return 0
    }
}
...
function allocate(e, r, t, n) {
    ...
    var i, s = "string" == typeof r ? r : null;
    if (i = t == ALLOC_NONE ? n : [_malloc, stackAlloc, dynamicAlloc][t](Math.max(a, s ? 1 : r.length)), o) {
        var u;
        for (n = i, assert(!(3 & i)), u = i + (-4 & a); n < u; n += 4) HEAP32[n >> 2] = 0;
        for (u = i + a; n < u;) HEAP8[0 | n++] = 0;
        return i
    }
    if ("i8" === s) return e.subarray || e.slice ? HEAPU8.set(e, i) : HEAPU8.set(new Uint8Array(e), i), i;
    for (var l, c, d, f = 0; f < a;) {
        var m = e[f];
        0 !== (l = s || r[f]) ? ("i64" == l && (l = "i32"), setValue(i + f, m, l), d !== l && (c = getNativeTypeSize(l), d = l), f += c) : f++
    }
    return i
}
...
function writeAsciiToMemory(e, r, t) {
    for (var n = 0; n < e.length; ++n) HEAP8[0 | r++] = e.charCodeAt(n);
    t || (HEAP8[0 | r] = 0)
}
...
})) && (buffer = wasmMemory.buffer), INITIAL_INITIAL_MEMORY = buffer.byteLength, updateGlobalBufferAndViews(buffer), HEAP32[DYNAMICTOP_PTR >> 2] = DYNAMIC_BASE;
var __ATPRERUN__ = [],
    __ATINIT__ = [],
    __ATMAIN__ = [],
    __ATPOSTRUN__ = [],
...
var tempDouble, tempI64, wasmBinaryFile = "rlottie-wasm.wasm";
...
function getBinaryPromise() {
    return wasmBinary || !ENVIRONMENT_IS_WEB && !ENVIRONMENT_IS_WORKER || "function" != typeof fetch || isFileURI(wasmBinaryFile) ? new Promise(function(e, r) {
        e(getBinary())
...
function createWasm() {
    ...
    return function() {
        if (wasmBinary || "function" != typeof WebAssembly.instantiateStreaming || isDataURI(wasmBinaryFile) || isFileURI(wasmBinaryFile) || "function" != typeof fetch) return n(t);
        fetch(wasmBinaryFile, {
            credentials: "same-origin"
        }).then(function(r) {
            return WebAssembly.instantiateStreaming(r, e).then(t, function(e) {
                err("wasm streaming compile failed: " + e), err("falling back to ArrayBuffer instantiation"), n(t)
            })
        })
    }(), {}
}
...
function __ZN7VRegionC1ERK5VRect() {
    err("missing function: _ZN7VRegionC1ERK5VRect"), abort(-1)
}

function __ZN7VRegionD1Ev() {
    err("missing function: _ZN7VRegionD1Ev"), abort(-1)
}

function __ZN7VRegionpLERK5VRect() {
    err("missing function: _ZN7VRegionpLERK5VRect"), abort(-1)
}

function __ZNK7VRegion12boundingRectEv() {
    err("missing function: _ZNK7VRegion12boundingRectEv"), abort(-1)
}
...
isDataURI(wasmBinaryFile) || (wasmBinaryFile = locateFile(wasmBinaryFile)), __ATINIT__.push({
    func: function() {
        ___wasm_call_ctors()
    }
});
var PATH = {
    ...
    MEMFS = {
        ...
        stream_ops: {
            ...
            write: function(e, r, t, n, o, a) {
                ...
                if (MEMFS.expandFileStorage(i, o + n), i.contents.subarray && r.subarray) i.contents.set(r.subarray(t, t + n), o);
                else
            ...
            mmap: function(e, r, t, n, o, a, i) {
                if (!FS.isFile(e.node.mode)) throw new FS.ErrnoError(43);
                var s, u, l = e.node.contents;
                if (2 & i || l.buffer !== r.buffer) {
                    (o > 0 || o + n < l.length) && (l = l.subarray ? l.subarray(o, o + n) : Array.prototype.slice.call(l, o, o + n)), u = !0;
    ...
    FS = {
        ...
        lookupPath: function(e, r) {
            if (r = r || {}, !(e = PATH_FS.resolve(FS.cwd(), e))) return {
                path: "",
                node: null
            };
            var t = {
            ...
            for (var o = PATH.normalizeArray(e.split("/").filter(function(e) {
                ...
                var u = s === o.length - 1;
                if (u && r.parent) break;
                if (a = FS.lookupNode(a, o[s]), i = PATH.join2(i, o[s]), FS.isMountpoint(a) && (!u || u && r.follow_mount) && (a = a.mounted.root), !u || r.follow)
                    for (var l = 0; FS.isLink(a.mode);) {
                        var c = FS.readlink(i);
                        if (i = PATH_FS.resolve(PATH.dirname(i), c), a = FS.lookupPath(i, {
        ...
        mount: function(e, r, t) {
            ...
            if (!o && !a) {
                ...
                if (t = i.path, n = i.node, FS.isMountpoint(n)) throw new FS.ErrnoError(10);
        ...
        rename: function(e, r) {
            ...
            if (!t || !n) throw new FS.ErrnoError(44);
            if (t.mount !== n.mount) throw new FS.ErrnoError(75);
            var u, l = FS.lookupNode(t, i),
                c = PATH_FS.relative(e, a);
            if ("." !== c.charAt(0)) throw new FS.ErrnoError(28);
            if ("." !== (c = PATH_FS.relative(r, o)).charAt(0)) throw new FS.ErrnoError(55);
            try {
                u = FS.lookupNode(n, s)
            } catch (e) {}
            if (l !== u) {
                ...
                if (FS.isMountpoint(l) || u && FS.isMountpoint(u)) throw new FS.ErrnoError(10);
                if (n !== t && (f = FS.nodePermissions(t, "w"))) throw new FS.ErrnoError(f);
                try {
        ...
        chmod: function(e, r, t) {
            ...
            if (!(n = "string" == typeof e ? FS.lookupPath(e, {
                    follow: !t
                }).node : e).node_ops.setattr) throw new FS.ErrnoError(63);
        ...
        ftruncate: function(e, r) {
            var t = FS.getStream(e);
            if (!t) throw new FS.ErrnoError(8);
            if (!(2097155 & t.flags)) throw new FS.ErrnoError(28);
        ...
        open: function(e, r, t, n, o) {
            ...
            var a;
            if (t = void 0 === t ? 438 : t, t = 64 & (r = "string" == typeof r ? FS.modeStringToFlags(r) : r) ? 4095 & t | 32768 : 0, "object" == typeof e) a = e;
            else {
                e = PATH.normalize(e);
                try {
                    a = FS.lookupPath(e, {
            ...
            if (64 & r)
                ...
                } else a = FS.mknod(e, t, 0), i = !0;
            if (!a) throw new FS.ErrnoError(44);
            if (FS.isChrdev(a.mode) && (r &= -513), 65536 & r && !FS.isDir(a.mode)) throw new FS.ErrnoError(54);
            if (!i) {
                var s = FS.mayOpen(a, r);
                if (s) throw new FS.ErrnoError(s)
            }
            512 & r && FS.truncate(a, 0), r &= -131713;
            ...
            u.stream_ops.open && u.stream_ops.open(u), !Module.logReadFiles || 1 & r || (FS.readFiles || (FS.readFiles = {}), e in FS.readFiles || (FS.readFiles[e] = 1, err("FS.trackingDelegate error on read file: " + e)));
            try {
                if (FS.trackingDelegate.onOpenFile) {
                    var l = 0;
                    1 != (2097155 & r) && (l |= FS.tracking.openFlags.READ), 2097155 & r && (l |= FS.tracking.openFlags.WRITE), FS.trackingDelegate.onOpenFile(e, l)
                }
            } catch (r) {
        ...
        readFile: function(e, r) {
            ...
            var t, n = FS.open(e, r.flags),
                o = FS.stat(e).size,
                a = new Uint8Array(o);
            return FS.read(n, a, 0, o, 0), "utf8" === r.encoding ? t = UTF8ArrayToString(a, 0) : "binary" === r.encoding && (t = a), FS.close(n), t
        },
        ...
        findObject: function(e, r) {
            ...
            return t.exists ? t.object : (setErrNo(t.error), null)
        },
        ...
        createDataFile: function(e, r, t, n, o, a) {
            ...
            if (t) {
                ...
                FS.chmod(u, 146 | s);
                var f = FS.open(u, "w");
                FS.write(f, t, 0, t.length, 0, a), FS.close(f), FS.chmod(u, s)
            }
            return u
        },
        ...
        createPreloadedFile: function(e, r, t, n, o, a, i, s, u, l) {
            Browser.init();
            ...
            function f(t) {
                function f(t) {
                    l && l(), s || FS.createDataFile(e, r, t, n, o, u), a && a(), removeRunDependency(d)
                }
                var m = !1;
        ...
        loadFilesFromDB: function(e, r, t) {
            ...
            o.onupgradeneeded = t, o.onsuccess = function() {
                ...
                e.forEach(function(e) {
                    var r = i.get(e);
                    r.onsuccess = function() {
                        FS.analyzePath(e).exists && FS.unlink(e), FS.createDataFile(PATH.dirname(e), PATH.basename(e), r.result, !0, !0, !0), ++s + u == l && c()
                    }, r.onerror = function() {
                        u++, s + u == l && c()
                    }
                }), a.onerror = t
    ...
    SYSCALLS = {
        ...
        doStat: function(e, r, t) {
            try {
                ...
                if (e && e.node && PATH.normalize(r) !== PATH.normalize(FS.getPath(e.node))) return -54;
                throw e
            }
            return HEAP32[t >> 2] = n.dev, HEAP32[t + 4 >> 2] = 0, HEAP32[t + 8 >> 2] = n.ino, HEAP32[t + 12 >> 2] = n.mode, HEAP32[t + 16 >> 2] = n.nlink, HEAP32[t + 20 >> 2] = n.uid, HEAP32[t + 24 >> 2] = n.gid, HEAP32[t + 28 >> 2] = n.rdev, HEAP32[t + 32 >> 2] = 0, tempI64 = [n.size >>> 0, (tempDouble = n.size, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (0 | Math_min(+Math_floor(tempDouble / 4294967296), 4294967295)) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[t + 40 >> 2] = tempI64[0], HEAP32[t + 44 >> 2] = tempI64[1], HEAP32[t + 48 >> 2] = 4096, HEAP32[t + 52 >> 2] = n.blocks, HEAP32[t + 56 >> 2] = n.atime.getTime() / 1e3 | 0, HEAP32[t + 60 >> 2] = 0, HEAP32[t + 64 >> 2] = n.mtime.getTime() / 1e3 | 0, HEAP32[t + 68 >> 2] = 0, HEAP32[t + 72 >> 2] = n.ctime.getTime() / 1e3 | 0, HEAP32[t + 76 >> 2] = 0, tempI64 = [n.ino >>> 0, (tempDouble = n.ino, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (0 | Math_min(+Math_floor(tempDouble / 4294967296), 4294967295)) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[t + 80 >> 2] = tempI64[0], HEAP32[t + 84 >> 2] = tempI64[1], 0
        },
        ...
        doReadlink: function(e, r, t) {
            ...
            var n = FS.readlink(e),
                ...
                a = HEAP8[r + o];
            return stringToUTF8(n, r, t + 1), HEAP8[r + o] = a, o
        },
...
function syscallMunmap(e, r) {
    if (-1 == (0 | e) || 0 === r) return -28;
    var t = SYSCALLS.mappings[e];
    if (!t) return 0;
    if (r === t.len) {
        var n = FS.getStream(t.fd);
        2 & t.prot && SYSCALLS.doMsync(e, n, r, t.flags, t.offset), FS.munmap(n), SYSCALLS.mappings[e] = null, t.allocated && _free(t.malloc)
    }
    return 0
}
...
function _saveSetjmp(e, r, t, n) {
    r |= 0, t |= 0, n |= 0;
    var o = 0;
    for (setjmpId = setjmpId + 1 | 0, HEAP32[(e |= 0) >> 2] = setjmpId;
        (0 | o) < (0 | n);) {
        if (!(0 | HEAP32[t + (o << 3) >> 2])) return HEAP32[t + (o << 3) >> 2] = setjmpId, HEAP32[t + (4 + (o << 3)) >> 2] = r, HEAP32[t + (8 + (o << 3)) >> 2] = 0, setTempRet0(0 | n), 0 | t;
        o = o + 1 | 0
    }
    return t = 0 | _saveSetjmp(0 | e, 0 | r, 0 | (t = 0 | _realloc(0 | t, 8 * (1 + (n = 2 * n | 0) | 0) | 0)), 0 | n), setTempRet0(0 | n), 0 | t
}

function _testSetjmp(e, r, t) {
    e |= 0, r |= 0, t |= 0;
    for (var n = 0, o = 0;
        (0 | n) < (0 | t) && 0 | (o = 0 | HEAP32[r + (n << 3) >> 2]);) {
        if ((0 | o) == (0 | e)) return 0 | HEAP32[r + (4 + (n << 3)) >> 2];
        n = n + 1 | 0
    }
    return 0
}
...
function _emscripten_longjmp(e, r) {
    _longjmp(e, r)
}
...
function _emscripten_resize_heap(e) {
    ...
    for (var n = 1; n <= 4; n *= 2) {
        var o = r * (1 + .2 / n);
        if (o = Math.min(o, e + 100663296), emscripten_realloc_buffer(Math.min(t, alignUp(Math.max(16777216, e, o), 65536)))) return !0
    }
    return !1
}
var ENV = {};
...
function _environ_get(e, r) {
    ...
    return getEnvStrings().forEach(function(n, o) {
        var a = r + t;
        HEAP32[e + 4 * o >> 2] = a, writeAsciiToMemory(n, a), t += n.length + 1
    }), 0
}
...
function _fd_seek(e, r, t, n, o) {
    try {
        var a = SYSCALLS.getStreamFromFD(e),
            i = 4294967296 * t + (r >>> 0),
            s = 9007199254740992;
        return i <= -s || i >= s ? -61 : (FS.llseek(a, i, n), tempI64 = [a.position >>> 0, (tempDouble = a.position, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (0 | Math_min(+Math_floor(tempDouble / 4294967296), 4294967295)) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[o >> 2] = tempI64[0], HEAP32[o + 4 >> 2] = tempI64[1], a.getdents && 0 === i && 0 === n && (a.getdents = null), 0)
    } catch (e) {
        return void 0 !== FS && e instanceof FS.ErrnoError || abort(e), e.errno
    }
}
...
function _strftime(e, r, t, n) {
    ...
    var _ = {
        ...
        "%V": function(e) {
            ...
            return m(i, o) < 0 ? "53" : m(a, i) <= 0 ? "01" : (r = o.getFullYear() < e.tm_year + 1900 ? e.tm_yday + 32 - o.getDate() : e.tm_yday + 1 - o.getDate(), f(Math.ceil(r / 7), 2))
        },
        "%w": function(e) {
            return e.tm_wday
        },
    ...
    for (var u in _) i.indexOf(u) >= 0 && (i = i.replace(new RegExp(u, "g"), _[u](a)));
    var F = intArrayFromString(i, !1);
...
var calledRun, ___wasm_call_ctors = Module.___wasm_call_ctors = function() {
    ...
    _lottie_init = Module._lottie_init = function() {
        return (_lottie_init = Module._lottie_init = Module.asm.D).apply(null, arguments)
    },
    _lottie_resize = Module._lottie_resize = function() {
...
if (Module.asm = asm, Module.intArrayFromString = intArrayFromString, Module.cwrap = cwrap, Module.allocate = allocate, dependenciesFulfilled = function e() {
